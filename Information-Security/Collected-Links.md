# A List of useful web sites concerning Information Security

## Overview sites 

- [https://digital-defense.io/](https://digital-defense.io/)

  *large collection of check lists on various security topics*

- [National Institute of Standards and Technology (NIST)](https://www.nist.gov/)
  - [Cybersecurity and Privacy](https://www.nist.gov/cybersecurity-and-privacy)
    - [NIST CyberSecurityFramework](https://www.nist.gov/cyberframework)
    - [NIST Privacy Framework](https://www.nist.gov/privacy-framework)
    - [NIST Risk Management Framework](https://csrc.nist.gov/projects/risk-management/about-rmf)
  - [Zero Trust Architecture](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-207.pdf)
- [Cybersecurity & Infrastructure Security Agency (CISA)](https://www.cisa.gov/)
  - [CISA Known Exploited Vulnerabilities Catalog (KEV)](https://www.cisa.gov/known-exploited-vulnerabilities-catalog)
  - [Zero Trust Maturity Model](https://www.cisa.gov/sites/default/files/2023-04/zero_trust_maturity_model_v2_508.pdf)



## useful other ressources

- List of obsolete / outdated software
  - https://endoflife.software/

## Articles on Linux Security

### Linux Hardening

- [A guide to infrastructure hardening](https://pages.ubuntu.com/rs/066-EOV-335/images/A_guide_to_infrastructure_hardening_WP_06.01.23.pdf) Canonical, 01/2023

  *Best practices to improve the security posture of your Linux-based infrastructure deployments.*

### Fail2ban

- https://codenet.dev/guide-to-whitelisting-banning-or-unbanning-an-ip-in-fail2ban/
- https://thelinuxcode.com/whitelist-ip-address-fail2ban/
- https://www.hostinger.com/tutorials/fail2ban-configuration

## Articles on Windows Security

- [Das Hochsicherheits-Windows](https://www.heise.de/select/ct/2023/20/2317914255711479609#!), heise select, Paywall

## Attacking Windows

- [Running EZ Tools Natively on Linux: A Step-by-Step Guide](https://www.sans.org/blog/running-ez-tools-natively-on-linux-a-step-by-step-guide/)

## Eye catching stuff

- [Browser-in-Browser-Attack](https://becker.koeln/videos/2024-10-19-brower-in-browser-attacks/) German Video by [Felix Becker](https://www.linkedin.com/in/dev-felix-becker/)

## online available books

- [Ross Anderson, Security Engineering – A guide to building dependable distributed Systems; 3rd Edition](https://www.cl.cam.ac.uk/archive/rja14/Papers/SEv3.pdf)

## Privacy

- [Database with Privary related fees](https://www.enforcementtracker.com/)

  *The CMS.Law GDPR Enforcement Tracker is an overview of fines and penalties which data protection authorities within the EU have imposed under the EU General Data Protection Regulation (GDPR, DSGVO).*
