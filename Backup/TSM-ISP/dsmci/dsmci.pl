# changelog
# date          version	 remark
# 2024-09-26    	 rebublished in Bjørn's own github Repo
#                        replace tabs with 9 blanks
# 2023-04-28    0.6.14   put changelog to top of file
# 2022-12-08    0.6.13   put suffixes for errorlog and stats into variables
...
# 2018-02-13    0.6.1    first version working, but profiling skipped out
# 2018-02-05    0.6.     fork from dsmcis.pl
...
my        $datatransfertime             = 0.0;          # total data transfer time in seconds
...
my        $errorlogapx                  = ".dsmci-error.txt";        # Appendix for error log file
...
my        $statfileapx                  = ".dsmci-stats.txt"; # Appendix for status file
...
if ( defined $CFGFILE )
{
	...
	while ($line = <$CFGFILE>)
        {
                # skip all lines starting with a comment sign
                if	( $line =~ /^\*/ or  $line =~ /^#/)
                {        next;        }
...
#####################################################################################
#
#        read last profiling and reorder folders due to last runtime
#
#####################################################################################
...
$errorlogfilename        = File::Spec->canonpath($date.$errorlogapx);
...
$statfilename             = File::Spec->canonpath($date.$statfileapx);
...
if ( $debugmode != TRUE )
{
        unlink $globallog_filename;        # remove global logfile gathred from partial logs
        remove_log_files();                # remove all partial log files
}
...
sub fork_backup_threads
{
        ...
        foreach $item (@allpaths)
        {
                ...
                {        # NO, they don't
                        ...
                        {        # unable to fork
                                $threadfail++;
                                if ($threadfail <= THREADFAILMAX)        # retry until THREADFAILMAX reached
                                {        
                                        sleep 30;
                                        redo;
                                }
                        ...
                        else                                            # child process
                        {
                                ...
                                close $GLOBALLOGFILE; # no need to use "flock" routine, as "close" also removes lock
